  ...
  private static float[] createVertices() {
    ...
    for (int j = 0; j < YLAT; ++j) {
      ...
      for (int i = 0; i < XLONG + 2; ++i) {
        ...
        if (i == XLONG) {
          vertices[base + i * step] = 0;
          vertices[base + i * step + 1] = 0;
          vertices[base + i * step + 2] = 0;
          vertices[base + i * step + 3] = 0;
          vertices[base + i * step + 4] = 0;
          vertices[base + i * step + 5] = 0;
        } else if (i == XLONG + 1) {
          vertices[base + i * step] = 0;
  ...
  private static int[] createIndices() {
    ...
    for (int j = 0; j < 2; j++) {
      for (int i = 0; i < XLONG - 1; i++) {
        ...
        indices[base + i * 3] = j * XLONG + i;
        indices[base + i * 3 + 1] = j * XLONG + i + 1;
        indices[base + i * 3 + 2] = vertices.length - 15 + j * 8;
      }
    }
